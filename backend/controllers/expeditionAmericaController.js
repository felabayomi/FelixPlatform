...
exports.generateDaily = async (_req, res) => {
    ...
    try {
        ...
        const target = await selectEditorialTargetForDate(tomorrow);
        send({
            type: 'selection',
            states: [target.state],
            total: 1,
            message: `Selected ${target.state.name} for next draft (${target.preferredCategory} priority).`,
...
const autoGenerateDraftForTomorrow = async () => {
    ...
    try {
        ...
        console.log(
            `[ExpeditionAmerica] Auto-generated draft for ${target.state.name} (${tomorrow}) with `
            + `${target.preferredCategory} priority`
        );
    } catch (error) {
