...
const CATEGORY_LIST = Array.from(ARTICLE_CATEGORIES);

const TABLE_NAME = 'expedition_america_articles';
const DAILY_DRAFT_LOCK_KEY = 41752061;
let ensureTablePromise = null;
let expeditionSchedulerStarted = false;
let lastAutoPublishDate = '';
let lastAutoGenerateDate = '';
const STATE_COOLDOWN_DAYS = Number(process.env.EXPEDITION_AMERICA_STATE_COOLDOWN_DAYS || 21);
const CATEGORY_WINDOW_DAYS = Number(process.env.EXPEDITION_AMERICA_CATEGORY_WINDOW_DAYS || 30);
const HISTORY_LOOKBACK_DAYS = Math.max(STATE_COOLDOWN_DAYS + 14, 45);
...
const getDateDaysAgo = (days, referenceYmd = getTodayDate()) => {
    const reference = parseYmdDate(referenceYmd) || new Date();
    const previous = new Date(reference.getTime() - (Math.max(0, Number(days) || 0) * 24 * 60 * 60 * 1000));
...
const selectEditorialTargetForDate = async (targetDate) => {
    ...
    const categoryWindowStart = getDateDaysAgo(CATEGORY_WINDOW_DAYS, normalizedTargetDate);

    const [coveredResult, historyResult, eventResults] = await Promise.all([
        pool.query(`SELECT state_code FROM ${TABLE_NAME} WHERE published_date = $1`, [normalizedTargetDate]),
        pool.query(
            `SELECT state_code, category, published_date
             FROM ${TABLE_NAME}
             WHERE published_date >= $1
               AND status IN ('draft', 'published')`,
            [historyStart]
        ),
        buildSearchResults(`major US travel events festivals tourism ${normalizedTargetDate}`),
    ]);

    const covered = new Set(coveredResult.rows.map((row) => row.state_code));
    const availableStates = US_STATES.filter((state) => !covered.has(state.code));
    if (!availableStates.length) {
        throw new Error(`All states already have coverage for ${normalizedTargetDate}`);
    }

    const latestByState = new Map();
    const categoryCounts = new Map(CATEGORY_LIST.map((category) => [category, 0]));

    historyResult.rows.forEach((row) => {
        const stateCode = String(row.state_code || '').trim().toUpperCase();
        const category = normalizeText(row.category);
        const publishedDate = normalizeText(row.published_date);

        if (!stateCode || !publishedDate) {
            return;
        }

        const currentLatest = latestByState.get(stateCode);
        if (!currentLatest || publishedDate > currentLatest) {
            latestByState.set(stateCode, publishedDate);
        }

        if (category && categoryCounts.has(category) && publishedDate >= categoryWindowStart) {
            categoryCounts.set(category, (categoryCounts.get(category) || 0) + 1);
        }
    });

    const minimumCategoryCount = Math.min(...Array.from(categoryCounts.values()));
    const categoryCandidates = CATEGORY_LIST.filter((category) => (categoryCounts.get(category) || 0) === minimumCategoryCount);
    const preferredCategory = categoryCandidates[Math.floor(Math.random() * categoryCandidates.length)] || 'Seasonal Highlights';

    const eventText = eventResults
        .map((item) => `${item.title || ''} ${item.snippet || ''}`.toLowerCase())
        .join(' ');

    const eligibleByCooldown = availableStates.filter((state) => {
        const lastDate = latestByState.get(state.code);
        if (!lastDate) {
            return true;
        }

        const daysSince = daysBetweenYmd(lastDate, normalizedTargetDate);
        return daysSince === null || daysSince >= STATE_COOLDOWN_DAYS;
    });

    const candidatePool = eligibleByCooldown.length ? eligibleByCooldown : availableStates;
    const scoredCandidates = candidatePool.map((state) => {
        const lastDate = latestByState.get(state.code) || null;
        const daysSince = lastDate ? (daysBetweenYmd(lastDate, normalizedTargetDate) ?? 0) : 365;
        const eventMatch = eventText.includes(state.name.toLowerCase()) || eventText.includes(` ${state.code.toLowerCase()} `);

        let score = Math.min(daysSince, 120);
        if (!lastDate) {
            score += 100;
        }
        if (eventMatch) {
    ...
    const topCandidates = scoredCandidates.filter((entry) => entry.score === topScore);
    ...
    return {
        ...
        preferredCategory,
        reason: {
            cooldownPoolUsed: eligibleByCooldown.length > 0,
            eventMatch: chosen.eventMatch,
            daysSinceLastCoverage: chosen.lastDate ? chosen.daysSince : null,
            preferredCategory,
        },
    };
};

const generateArticleForState = async (stateCode, options = {}) => {
    if (!hasAiConfigured()) {
    ...
    const preferredCategory = ARTICLE_CATEGORIES.has(options.preferredCategory)
    ...
    return {
        ...
        category: ARTICLE_CATEGORIES.has(parsed.category)
            ? parsed.category
            : (preferredCategory || 'Seasonal Highlights'),
...
exports.generateDaily = async (_req, res) => {
    ...
    try {
        ...
        const result = await createDailyDraftIfMissing(tomorrow);
        ...
        if (!result.created) {
            if (result.reason === 'already_exists') {
                send({
                    ...
                    completed: 0,
                    failed: 0,
                    message: `Skipped generation: an article already exists for ${tomorrow}.`,
                    skipped: true,
                ...
                send({
                    ...
                    completed: 0,
                    failed: 0,
                    message: 'Skipped generation: another generation process is currently running.',
                    skipped: true,
                });
            }
            return res.end();
        ...
        send({
            ...
            total: 1,
            message: `Selected ${result.target.state.name} for next draft (${result.target.preferredCategory} priority).`,
            preferredCategory: result.target.preferredCategory,
            strategy: result.target.reason,
        });
        send({
            type: 'state_complete',
            stateCode: result.target.state.code,
            stateName: result.target.state.name,
            completed: 1,
            total: 1,
...
const autoGenerateDraftForTomorrow = async () => {
    ...
    try {
        ...
        if (!result.created) {
            if (result.reason === 'already_exists') {
                console.log(`[ExpeditionAmerica] Skipped auto-generation for ${tomorrow}: draft already exists.`);
            } else {
                console.log(`[ExpeditionAmerica] Skipped auto-generation for ${tomorrow}: generation lock held.`);
            }
            return;
        }

        console.log(
            `[ExpeditionAmerica] Auto-generated draft for ${result.target.state.name} (${tomorrow}) with `
            + `${result.target.preferredCategory} priority`
        );
    } catch (error) {
...
const createDailyDraftIfMissing = async (targetDate) => {
    const normalizedTargetDate = normalizeText(targetDate, getTomorrowDate());
    ...
    try {
        ...
        const lockResult = await pool.query('SELECT pg_try_advisory_lock($1) AS locked', [DAILY_DRAFT_LOCK_KEY]);
        lockAcquired = Boolean(lockResult.rows[0]?.locked);
        if (!lockAcquired) {
            return { created: false, reason: 'locked' };
        }

        const existingResult = await pool.query(
            `SELECT id, state_name, title, status
             FROM ${TABLE_NAME}
             WHERE published_date = $1
               AND status IN ('draft', 'published')
        ...
        const target = await selectEditorialTargetForDate(normalizedTargetDate);
        const articleData = await generateArticleForState(target.state.code, {
            targetDate: normalizedTargetDate,
            preferredCategory: target.preferredCategory,
        });
        ...
        if (lockAcquired) {
            await pool.query('SELECT pg_advisory_unlock($1)', [DAILY_DRAFT_LOCK_KEY]).catch(() => { });
        }
    }
};